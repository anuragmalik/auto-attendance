...
public class AttChart extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String res="<table width=\"100%\"><tr><th colspan=\"4\" bgcolor=\"#c0c0c0\">This Week</th>"+
    ...
    private void drawWeek(String suid, String res, HttpServletResponse response) throws IOException
    {
		...
		try { 
			...
			q="SELECT SUB_NAME,COLNO,LECTURE_COUNT FROM Anurag.SUBJECTS,Anurag.STUDENT WHERE SUID='"+suid
					+"'AND Anurag.SUBJECTS.TCODE=Anurag.STUDENT.TCODE";
			java.sql.ResultSet rs = st.executeQuery(q);
		    while(rs.next()){
		    	java.sql.Statement str = con.createStatement();
		    	String w="SELECT "+rs.getString("COLNO").trim() +" FROM Anurag."+slp[1]+"_ATTENDANCE where suid='"+suid+"'";
		    	java.sql.ResultSet rsr = str.executeQuery(w);
		    	while(rsr.next()){
		    		int att=Integer.parseInt(rsr.getString(1).trim());
		    		total +=att;
		    		int lcount=Integer.parseInt(rs.getString("LECTURE_COUNT").trim());
		    		totlec +=lcount;
		    		double per;
		    		if(lcount!=0)
		    		{per=(att*100)/lcount;}
		    		...
		    		res +="<tr><td>"+rs.getString("SUB_NAME").trim()+"</td><td>"+lcount+"</td><td>"+att+"</td><td>"
		    		               +per+"</td></tr>";
		    		} 			}
		    ...
		    res +="<tr><td>Over All</td><td>"+totlec+"</td><td>"+total+"</td><td>"+pert+"</td></tr>";
		        ...
		        chart.setTitle("Weekly Attendance", FIREBRICK, 14);
